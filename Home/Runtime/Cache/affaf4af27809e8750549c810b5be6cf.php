				...
				<div class="row">
					<table class="table table-hover table-bordered table-responsive" id="blogtable">
					  <thead>
					   ...
					   <tbody id="tb_b_cbAll">  
					    <?php if($blogcontent == null): ?><tr>
						      <td colspan="7" class="text-center text-muted">
							    <p>暂无数据。。。。。。</p>
						      </td>
					        </tr>
					    <?php else: ?>
						<?php if(is_array($blogcontent)): foreach($blogcontent as $key=>$con): ?><tr>
								...
								<td class="text-center"><?php echo (date("Y-m-d",$con['b_pubtime'])); ?></td>
   ...
   <script type="text/javascript">
        ...
        function blogedit(a){
          ...
          $.ajax({
          	...
          	success:function(b){
                 ...
                 editor.html(b[0]['b_content']);//向kindeditornt']);
          	}
          });
        }
       ...
       /*3.拟态框编辑博文开始*/
        $("#blogupdate").click(function(){
            ...
            var blogkindeditor=editor.html();
             //alert(blogselect);
            $.ajax({
              type:"post",

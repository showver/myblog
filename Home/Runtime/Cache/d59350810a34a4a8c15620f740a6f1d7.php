	...
	<div id="blog_content" class="container-fluid">			
		...
		<!-- aside Section -->
			...
			<article class="mana_comment_a col-xs-10">
				...
				<hr>
				<div class="row">
					<form action="" class="form-inline" action="__URL__/blognoshenkey" method="get">
				...
				<div class="row">
					<table class="table table-hover table-bordered table-responsive">
					   ...
					   <tbody id="tb_p_cbAll">
					   <?php if($nopingsou == null): ?><tr>
								<td colspan="7" class="text-center text-muted">
									<p>暂无数据。。。。。。</p>
								</td>
					        </tr>
					   <?php else: ?>
						<?php if(is_array($nopingsou)): foreach($nopingsou as $key=>$s): ?><tr>
								<td class="text-center">
									<input type="checkbox" onClick="checkbox('p_cbAll')" class="box_p_cbAll" value="<?php echo ($s['p_id']); ?>">
								</td>
								<td class="text-center"><?php echo ($s['p_id']); ?></td>
								<td class="text-center"><?php echo ($s['p_content']); ?></td>
								<td class="text-center"><b><font color='red'><?php echo (date("Y-m-d",$s['p_pubtime'] )); ?></font></b></td>
								<td class="text-center"><?php echo ($s['user']); ?></td>
								<td class="text-center">
									<a href="__URL__/comm_check?p_id=<?php echo ($s['p_id']); ?>">
										<i class="icon-leaf btn btn-primary">查看</i>
									</a>

									<?php if($s['p_zhuangtai'] == '1'): ?><i id="renew<?php echo ($s['p_id']); ?>" onClick="renew(<?php echo ($s['p_zhuangtai']); ?>,<?php echo ($s['p_id']); ?>)" class="icon-remove btn btn-warning">屏蔽</i>
									<?php else: ?>
									  <i id="renew<?php echo ($s['p_id']); ?>" onClick="renew(<?php echo ($s['p_zhuangtai']); ?>,<?php echo ($s['p_id']); ?>)" class="icon-ok btn btn-success">恢复</i><?php endif; ?>
									
									<i class="icon-trash btn btn-danger" data-toggle="modal" data-target="#delect" onClick="pinglundel(<?php echo ($s['p_id']); ?>)">删除</i>
								</td>
							</tr><?php endforeach; endif; endif; ?>
					  </tbody>
					</table>
				</div>
				<?php if($page == '   '): if($nopingsou == null): else: ?>
...
<script type="text/javascript">
	...
	/*-----5、评论 搜索  ------*/
		/*function pinglunsearch(){
			var blogpinglunkey=$("#blogpinglunkey").val();
			var pingluntime=$("#pingluntime").val();
			//alert(pingluntime);
			$.ajax({
				type:"post",
				url:"blognoshenkey",
				data:{"nopinglunkey":blogpinglunkey,"nopingluntime":pingluntime},
				success:function(p){
					//alert(p);
					$("#tb_p_cbAll tr").remove();
					if(p!=null){
		             for(var i=0;i<p.length;i++){
		             	var p_content=p[i]['p_content'].replace(blogpinglunkey,"<font color='red'>"+blogpinglunkey+"</font>")
		             	var str="";
		             		str+="<tr>";
							str+="<td class='text-center'>";
							str+="<input type='checkbox' onClick='checkbox('p_cbAll')' class='box_p_cbAll' value='"+p[i]['p_id']+"'>";
							str+="</td>";
							str+="<td class='text-center'>"+p[i]['p_id']+"</td>";
							str+="<td class='text-center'>"+p_content+"</td>";
							str+="<td class='text-center'><font color='red'>"+p[i]['p_pubtime']+"</font></td>";
							str+="<td class='text-center'>"+p[i]['user']+"</td>";
							str+="<td class='text-center'>";
							str+="<a href='comm_check?p_id="+p[i]['p_id']+"'>";
							str+="<i class='icon-leaf btn btn-primary'>查看</i>";
							str+="</a>";

							if(p[i]['p_zhuangtai']==1){
							str+="<i id='renew"+p[i]['p_id']+"' onClick='renew("+p[i]['p_zhuangtai']+","+p[i]['p_id']+")' class='icon-remove btn btn-warning'>屏蔽</i>";
							}else{
							str+="<i id='renew"+p[i]['p_id']+"' onClick='renew("+p[i]['p_zhuangtai']+","+p[i]['p_id']+")' class='icon-ok btn btn-success'>恢复</i>";
							}
									
							str+="<i class='icon-trash btn btn-danger' data-toggle='modal' data-target='#delect' onClick='pinglundel("+p[i]['p_id']+")'>删除</i>";
							str+="</td>";
							str+="</tr>";
						$("#tb_p_cbAll").append(str);
		             }
